        ...
        >
          <el-form-item>
            ...
            <a
              ...
              v-if="isAuth('shangjiaReport', '导入导出')"
              icon="el-icon-download"
              href="http://localhost:8080/card_coupons/upload/shangjiaReportMuBan.xls"
              >批量导入卡券商品评价数据模板</a
            >
            &nbsp;
            <el-upload
              ...
              :action="getActionUrl"
              :on-success="shangjiaReportUploadSuccess"
              :on-error="shangjiaReportUploadError"
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span
                v-if="
                  scope.row.status != null && scope.row.status.length > 10
                "
              >
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      params["shangjiaReportDelete"] = 1; // 逻辑删除字段 1 未删除 2 删除

      this.$http({
        url: "shangjiaReport/page",
        method: "get",
        params: params,
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    // 导入功能上传文件成功后调用导入方法
    shangjiaReportUploadSuccess(data) {
      let _this = this;
      _this
        .$http({
          url: "shangjiaReport/batchInsert?fileName=" + data.file,
          method: "get",
        })
        .then(({ data }) => {
          if (data && data.code === 0) {
    ...
    // 导入功能上传文件失败后调用导入方法
    shangjiaReportUploadError(data) {
    ...
    wuyong(id) {
      ...
      _this
        ...
        .then(() => {
          _this
            .$http({
              url: "shangjiaReport/update",
              method: "post",
              data: {
                id: id,
                //                            shangjiaReportTypes:shangjiaReportTypes,
              },
            })
            .then(({ data }) => {
              if (data && data.code === 0) {
