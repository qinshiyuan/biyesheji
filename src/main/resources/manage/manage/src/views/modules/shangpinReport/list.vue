        ...
        >
          <el-form-item :label="contents.inputTitle == 1 ? '卡券商品名称' : ''">
            <el-input
              ...
              v-model="searchForm.shangpinName"
              placeholder="卡券商品名称"
              clearable
            ></el-input>
          </el-form-item>
        ...
        >
          <el-form-item>
            ...
            <a
              ...
              v-if="isAuth('shangpinReport', '导入导出')"
              icon="el-icon-download"
              href="http://localhost:8080/card_coupons/upload/shangpinReportMuBan.xls"
              >批量导入卡券商品评价数据模板</a
            >
            &nbsp;
            <el-upload
              ...
              :action="getActionUrl"
              :on-success="shangpinReportUploadSuccess"
              :on-error="shangpinReportUploadError"
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span
                v-if="
                  scope.row.reportReason != null &&
                  scope.row.reportReason.length > 10
                "
              >
                {{ scope.row.reportReason.slice(0, 10) }}...
              </span>
              <span v-else>
          ...
          >
            <template slot-scope="scope">
              <span
                v-if="scope.row.status != null && scope.row.status.length > 10"
              >
                {{ scope.row.status.slice(0, 10) }}...
              </span>
              <span v-else>
                {{ scope.row.status }}
              </span>
          ...
          >
            <template slot-scope="scope">
              ...
              <el-button
                v-if="
                  isAuth('shangpinReport', '处理')  &&
                  scope.row.status == '待处理' 
                "
                type="primary"
                icon="el-icon-edit"
                size="mini"
                @click="chuliHandler(scope.row.id)"
                >处理</el-button
              >
              <el-button
                v-if="isAuth('shangpinReport', '删除')"
  ...
  methods: {
    ...
    getDataList() {
      ...
      if (
        ...
        this.searchForm.shangpinName != undefined
      ) {
        params["shangpinName"] = "%" + this.searchForm.shangpinName + "%";
      }

      if (
        this.searchForm.shangpinTypes != "" &&
        this.searchForm.shangpinTypes != undefined
      ) {
      ...
      params["shangpinReportDelete"] = 1; // 逻辑删除字段 1 未删除 2 删除

      this.$http({
        url: "shangpinReport/page",
        method: "get",
        params: params,
      }).then(({ data }) => {
        if (data && data.code === 0) {
...
this.$http({
  ...
  if (data && data.code === 0) {
    ...
    // 导入功能上传文件成功后调用导入方法
    shangpinReportUploadSuccess(data) {
      let _this = this;
      _this
        .$http({
          url: "shangpinReport/batchInsert?fileName=" + data.file,
          method: "get",
        })
        .then(({ data }) => {
          if (data && data.code === 0) {
            _this.$message({
              message: "导入卡券商品评价数据成功",
              type: "success",
              duration: 1500,
              onClose: () => {
                _this.search();
    ...
    // 导入功能上传文件失败后调用导入方法
    shangpinReportUploadError(data) {
    ...
    wuyong(id) {
      ...
      _this
        ...
        .then(() => {
          _this
            .$http({
              url: "shangpinReport/update",
              method: "post",
              data: {
                id: id,
                //                            shangpinReportTypes:shangpinReportTypes,
              },
            })
            .then(({ data }) => {
              if (data && data.code === 0) {
