  ...
  <div class="addEdit-block">
    ...
    >
      <el-row>
        <el-col
          :span="12"
          v-if="sessionTable != 'shangpin' && sessionTable != 'shangjia'"
        >
          <el-form-item
            class="select"
            v-if="type != 'info'"
          ...
          >
            <el-select
              v-model="ruleForm.shangpinId"
              :disabled="ro.shangpinId"
              filterable
              placeholder="请选择卡券商品"
              @change="shangpinChange"
            >
              <el-option
        ...
        <el-col :span="12" v-if="sessionTable != 'shangpin'">
          ...
          >
            <el-input
              v-model="shangpinForm.shangpinName"
              placeholder="卡券商品名称"
              clearable
              readonly
            ></el-input>
        ...
        <el-col :span="24">
          ...
          >
            <el-input
              type="textarea"
              :readonly="ro.reportReason"
              v-model="ruleForm.reportReason"
              placeholder="举报原因"
            ></el-input>
          </el-form-item>
          <div v-else-if="type == 'info'">
            <el-form-item
              v-if="ruleForm.reportReason"
              label="举报原因"
              prop="reportReason"
            >
              <span v-html="ruleForm.reportReason"></span>
            </el-form-item>
          </div>
        </el-col>
        <el-col :span="24">
          <el-form-item v-if="type != 'info'" label="举报状态" prop="status">
            <el-input
              type="textarea"
              :readonly="ro.status"
          ...
          <div v-else-if="type == 'info'">
            <el-form-item
              ...
              prop="status"
            >
              <span v-html="ruleForm.status"></span>
            </el-form-item>
          </div>
        </el-col>
      </el-row>
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        status: [
          { required: true, message: "举报状态不能为空", trigger: "blur" },
        ],
        updateTime: [
  ...
  created() {
    ...
    if (this.role != "管理员") {
      this.ro.shangpinId = true; //卡券商品
      this.ro.yonghuId = true; //用户
      this.ro.reportReason = true; //举报原因
    }
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs["ruleForm"].validate((valid) => {
        if (valid) {
          this.$http({
            url: `shangpinReport/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm,
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shangpinReportCrossAddOrUpdateFlag = false;
                  this.parent.search();
