    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        >
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '订单类型' : ''">
            <el-select v-model="searchForm.shangpinOrderTypes" placeholder="请选择订单类型">
              ...
              <el-option v-for="(item, index) in shangpinOrderTypesSelectSearch" v-bind:key="index" :label="item.indexName" :value="item.codeIndex">
                <!--lable是要展示的名称-->
                <!--value是值-->
              </el-option>
            </el-select>
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '卡券商品名称' : ''">
            <el-input prefix-icon="el-icon-search" v-model="searchForm.shangpinName" placeholder="卡券商品名称" clearable></el-input>
          </el-form-item>

          <el-form-item :label="contents.inputTitle == 1 ? '卡券商品类型' : ''">
            <el-select v-model="searchForm.shangpinTypes" placeholder="请选择卡券商品类型">
              <el-option label="=-请选择-=" value=""></el-option>
              <el-option v-for="(item, index) in shangpinTypesSelectSearch" v-bind:key="index" :label="item.indexName" :value="item.codeIndex">
                <!--lable是要展示的名称-->
                <!--value是值-->
              </el-option>
            </el-select>
        ...
        <el-row
          class="ad"
          :style="{
            justifyContent: contents.btnAdAllBoxPosition == '1' ? 'flex-start' : contents.btnAdAllBoxPosition == '2' ? 'center' : 'flex-end',
          }"
        >
          <el-form-item>
            <el-button v-if="isAuth('shangpinOrder', '新增')" type="success" icon="el-icon-plus" @click="addOrUpdateHandler()">新增</el-button>
            &nbsp;
            <el-button
      ...
      <div class="table-content">
        ...
        >
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="addressName" header-align="center" label="收货人">
            <template slot-scope="scope">
              {{ scope.row.addressName }}
            </template>
          </el-table-column>
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="addressPhone" header-align="center" label="电话">
            <template slot-scope="scope">
              {{ scope.row.addressPhone }}
            </template>
          </el-table-column>
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="addressDizhi" header-align="center" label="地址">
            <template slot-scope="scope">
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yonghuName" header-align="center" label="用户姓名">
            <template slot-scope="scope">
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="buyNumber" header-align="center" label="购买数量">
            <template slot-scope="scope">
              {{ scope.row.buyNumber }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
            <template slot-scope="scope">
              ...
              <el-button v-if="sessionTable == 'false'" type="success" icon="el-icon-printer" size="mini" @click="dayinOpen(scope.row)"
                >打印</el-button
              >
              <el-button
                v-if="isAuth('shangpinOrder', '查看')"
              ...
              <el-button
                ...
                >修改</el-button
              >
              <el-button v-if="isAuth('shangpinOrder', '删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)"
                >删除</el-button
              >
              <el-button
                v-if="
                  isAuth('shangpinOrder', '订单') && scope.row.shangpinOrderTypes == 101 && sessionTable == 'yonghu' && userId == scope.row.yonghuId
                "
                type="primary"
              ...
              <el-button
                v-if="
                  isAuth('shangpinOrder', '订单') && scope.row.shangpinOrderTypes == 104 && sessionTable == 'yonghu' && userId == scope.row.yonghuId
                "
                type="primary"
              ...
              <el-button
                v-if="
                  isAuth('shangpinOrder', '订单') &&
                  scope.row.shangpinOrderTypes == 101 &&
                  sessionTable == 'shangjia' &&
                  userId == scope.row.shangpinShangjiaId
                "
                type="primary"
                icon="el-icon-reading"
                size="mini"
              ...
              <el-button
                v-if="
                  isAuth('shangpinOrder', '订单') && scope.row.shangpinOrderTypes == 103 && sessionTable == 'yonghu' && userId == scope.row.yonghuId
                "
                type="primary"
                icon="el-icon-reading"
                size="mini"
...
export default {
  data() {
    return {
      ...
      json_fields: {
        ...
        卡券商品名称: "shangpinName",
        卡券商品编号: "shangpinUuidNumber",
        卡券商品照片: "shangpinPhoto",
        卡券商品类型: "shangpinTypes",
        卡券商品库存: "shangpinKucunNumber",
        卡券商品原价: "shangpinOldMoney",
        现价: "shangpinNewMoney",
        卡券商品热度: "shangpinClicknum",
        是否上架: "shangxiaTypes",
        用户姓名: "yonghuName",
        联系方式: "yonghuPhone",
        身份证号: "yonghuIdNumber",
  ...
  methods: {
    ...
    deliver() {
      let _this = this;
      if (this.shangpinOrderCourierNumber == "" || this.shangpinOrderCourierNumber == null || this.shangpinOrderCourierNumber == "null") {
        return this.$message.error("订单快递单号不能为空");
      }
      if (this.shangpinOrderCourierName == "" || this.shangpinOrderCourierName == null || this.shangpinOrderCourierName == "null") {
      ...
      _this
        ...
        .then(() => {
          ...
          _this
            .$http({
              url:
                "shangpinOrder/deliver?id=" +
                id +
                "&shangpinOrderCourierNumber=" +
                _this.shangpinOrderCourierNumber +
                "&shangpinOrderCourierName=" +
                _this.shangpinOrderCourierName,
              method: "post",
            })
            .then(({ data }) => {
              if (data && data.code === 0) {
      ...
      _this
        ...
        .then(() => {
          _this
            .$http({
              url:
                "shangpinOrder/commentback?id=" +
                _this.commentbackId +
                "&commentbackText=" +
                _this.commentbackContent +
                "&shangpinCommentbackPingfenNumber=" +
                _this.shangpinCommentbackPingfenNumber,
              method: "post",
            })
            .then(({ data }) => {
              if (data && data.code === 0) {
    ...
    wuyong(id) {
      let _this = this;
      _this.$confirm(`确定    操作?`, "提示", { confirmButtonText: "确定", cancelButtonText: "取消", type: "warning" }).then(() => {
        _this
          .$http({
