<template>
	<view>
		...
		<mescroll-uni :top="88" @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback"
				...
				<view class="bg-white margin-sm padding-sm solids-bottom radius text-content">
					<view class="flex justify-between">
						<view class="text-bold" style="flex-shrink: 0;">
							<text
								class="text-green">{{ item.shangpinOrderTypes == 101?item.shangpinOrderValue:'' }}</text>
							<text
								class="text-red">{{ item.shangpinOrderTypes == 102?item.shangpinOrderValue:'' }}</text>
							<text
								class="text-grey">{{ item.shangpinOrderTypes == 103?item.shangpinOrderValue:'' }}</text>
							<text
								class="text-blue">{{ item.shangpinOrderTypes == 104?item.shangpinOrderValue:'' }}</text>
							<text
								class="text-brown">{{ item.shangpinOrderTypes == 105?item.shangpinOrderValue:'' }}</text>
						<text
							class="text-brown">{{ item.shangpinOrderTypes == 106?item.shangpinOrderValue:'' }}</text>
								
						</view>
						<view><text class="icon-shangpin padding-right-sm"></text><text
								class="text-bold solids-bottom">{{ item.shangpinOrderUuidNumber }}</text></view>
					</view>
					<view class="bg-white margin-xs padding-xs  radius text-content">
						<view class="flex padding-top-sm solids-top">
							<image :src="baseUrl+item.shangpinPhoto" style="flex-shrink: 0;" mode="aspectFill"
						...
						<view class="flex justify-end"
							...
							{{ item.shangpinOrderCourierName }}: <text class="text-bold solids-bottom">
								{{ item.shangpinOrderCourierNumber }}</text></view>
					...
					<view class="flex justify-end" style="font-size: 24upx;">
						...
						<view v-if="item.shangpinOrderTypes==104">
						 ...
						 <view v-if="item.shangpinOrderTypes==104">
						    <view @tap="reportback(item.id)" class="round cu-btn lines-grey mid margin-right-sm">举报</view>
						  </view>
			...
			<uni-popup class="popup-content" ref="report" type="center">
			    <form class="popup-form">
			      <view class="uni-flex uni-column"
			        style="background-color: #FFFFFF; border-radius: 10rpx; width: 500rpx; height: auto;">
			        <view class="flex-item flex-item-V" style="text-align: center; font-size: 40upx; font-weight: bold; margin: 20upx;">
			          举报
			        </view>
			        <view class="flex-item flex-item-V">
			          ...
			          <textarea
			            ...
			            v-model="shangpinReportReason" placeholder="举报内容"></textarea>
			        </view>
			        <view class="flex-item flex-item-V" style="text-align: right; background: #FFFFFF;">
			          <button @tap="onCloseReportTap" style="margin: 0 0 10rpx 50rpx;" class="cu-btn bg-cyan">取消</button>
			          <button @tap="onFinishReportTap" style="float: right; margin: 0 50rpx 10rpx 0;" class="cu-btn bg-red">确认</button>
			        </view>
			      </view>
			    </form>
			  </uni-popup>
				...
				<form class="popup-form">
					<view class="uni-flex uni-column"
						...
						<view class="flex-item flex-item-V">
							...
							<textarea
								style="width: 400rpx;margin: 10rpx 0 10rpx 50rpx;border-style:dotted;border-width:1rpx;border-radius: 10rpx"
								v-model="shangpinCommentbackText" placeholder="评论内容"></textarea>
						</view>
						<view class="flex-item flex-item-V" style="text-align: right;background: #FFFFFF;">
		...
		methods: {
			 // 打开举报弹窗
			    ...
			    async onFinishReportTap() {
			      ...
			      // 调用接口提交举报
			      await _this.$api.requestCondition("shangpinOrder", 'submitReport', data);
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							var returnVal = await _this.$api.requestCondition("shangpinOrder", "refund",
								value);
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							var returnVal = await _this.$api.requestCondition("shangpinOrder", "receiving",
								order);
			...
			async deliver(id) {
				let _this = this;
				if (_this.shangpinOrderCourierName == null || _this.shangpinOrderCourierName == "" || _this
					.shangpinOrderCourierName == "null") {
					return _this.$utils.msg('快递公司不能为空');
				}
				if (_this.shangpinOrderCourierNumber == null || _this.shangpinOrderCourierNumber == "" || _this
					.shangpinOrderCourierNumber == "null") {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							var returnVal = await _this.$api.requestCondition("shangpinOrder", "deliver",
								value);
