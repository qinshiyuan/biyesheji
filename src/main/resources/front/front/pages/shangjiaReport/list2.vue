	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.reportReason" type="text" placeholder="举报内容"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('shangjiaReport','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
                ...
                <view style="font-size: 30rpx;">
                    ...
                    <b>{{item.shangjiaName}}</b>
                        ...
                        <view class="address-box" style="padding: 10rpx;">
                            <text style="margin-left: 30rpx;">举报原因：{{item.reportReason}}</text>
                        </view>
                        <view class="u-box" style="color: #8799A3;" v-if="item.status!= null && item.status!= 'null' && item.status!= ''">
                            <text class="name" style="margin-left: 30rpx;font-size: 23rpx;">举报状态：{{item.status}}</text>
                        </view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`shangjiaReport`, params);
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`shangjiaReport`, {
                                id:id,
                            });
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('shangjiaReport', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`shangjiaReport`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.reportReason) {
                    searchForm['reportReason'] = this.searchForm.reportReason
                }

                let res = await this.$api.page(`shangjiaReport`, searchForm);
				// 如果是第一页数据置空
